...
# from myapp.api import api_util
# from myapp.api.api_util import response_helper, parser_helper, get_request, post_helper
# from myapp.api.api_util import post_scrape_helper, update_db_helper, delete_helper
from api_util import response_helper, parser_helper, get_request, post_helper, get_all_request
...
def get_all_book():
    """GET endpoint.get book  """
    limit_value = request.args['limit'].strip('"')
    limit_value = int(limit_value)
    try:
        data = get_all_request(True, False, limit_value)
        if data is None:
            return response_helper("cannot find Books", 500)
        return data, 200
...
def get_all_author():
    """GET endpoint.get all author book """
    """GET endpoint.get book  """
    limit_value = request.args['limit'].strip('"')
    limit_value = int(limit_value)
    try:
        data = get_all_request(False, False, limit_value)
        if data is None:
...
def vis_top_authors():
    """GET endpoint.get book  """
    try:
        data = get_all_request(False, True, 300)
        if data is None:
            return response_helper("cannot find authors", 500)
        sorted_tuples = sorted(data.items(), key=lambda item: item[1])
...
def post_one_author_info():
    ...
    if args.infile is None:
        new_dict = []
        new_dict.append(json.loads(request.data)["postAuthor"])
    else:
...
def post_one_book_info():
    ...
    new_dict = []
    new_dict.append(json.loads(request.data)["postBook"])
    try:
...
def put_book_info():
    ...
    if args.json_list is None:
        new_dict = json.loads(request.data)['dict']
    else:
    ...
    try:
        ...
        print("success")
        return response_helper("success updated", 200)
    except Exception:
        return response_helper("internal error", 500)
