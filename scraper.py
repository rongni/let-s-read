...
def main():
    ...
    parser.add_argument('--num_book', type=int, default=200)
    parser.add_argument('--num_author', type=int, default=50)
    parser.add_argument(
        '--url', type=str, default="https://www.goodreads.com/book/show/149267.The_Stand")
    ...
    book_next.append(args.url)
    while len(book_urls) < args.num_book or len(author_urls) < args.num_author:
        if len(book_next) == 0:
            logger.warning("Not hit required value")
            break
        to_scrape = book_next[0]
        scrap_book(to_scrape, args.num_author)
    ...
    if args.export_to_json:
        write_json(appened_book, 'book_test_6.json')
        write_json(appened_author, 'author_test_6.json')
    client.close()
    time2 = time.time()
